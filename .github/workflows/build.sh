...
case "$os" in
	"windows")
		...
		# Only Windows publishes debug symbols (the .pdb files in the build artifact),
		# so the cost of generating them is not paid anywhere else.
		OS_SPECIFIC_CMAKE_OPTIONS="$OS_SPECIFIC_CMAKE_OPTIONS -Dsymbols=ON"
